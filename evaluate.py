...
def prepare_input(rgb_path, sparse_path, positions_dir):
    ...
    sparse_tensor = load_sparse_png(sparse_path)/100
    ...
    return input_tensor.unsqueeze(0), h, w  
...
def main():
    ...
    for rgb_path in tqdm(sorted(input_dir.glob("*.png"))):
        name = rgb_path.stem
        sparse_path = sparse_dir / (name + ".png")

        input_tensor, h, w = prepare_input(rgb_path, sparse_path, pos_dir)
